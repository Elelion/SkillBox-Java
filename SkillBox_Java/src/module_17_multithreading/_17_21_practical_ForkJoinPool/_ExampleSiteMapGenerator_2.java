...
public class _ExampleSiteMapGenerator_2 {
    ...
    public _ExampleSiteMapGenerator_2() throws IOException, InterruptedException {
        this.visitedUrls = new HashSet<>();
        this.siteMap = new StringBuilder();

        Document doc = getUrlConnecting();
        generateSiteMap(doc);
    }

    /**/

    private Document getUrlConnecting() {
        try {
    ...
    private void visitChildPages(String url, int level) {
        try {
            ...
            */
            ...
            ExecutorService executor = Executors.newFixedThreadPool(CORES_CPU);
            List<Callable<Void>> tasks = new LinkedList<>();

            for (Element link : links) {
                tasks.add(new MyThread(link, level));
            }
    ...
    public class MyThread implements Callable<Void> {
        private Element link;
        private int level;

        public MyThread(Element link, int level) {
            this.link = link;
            this.level = level;
        }
